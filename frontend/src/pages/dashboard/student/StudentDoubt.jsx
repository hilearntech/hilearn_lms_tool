...
const StudentDoubt = () => {
  ...
  const fetchMyDoubts = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get("http://localhost:5000/api/doubts/my-doubts", {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`http://localhost:5000/api/doubts/delete/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post("http://localhost:5000/api/doubts/create", formData, {
        headers: { 
                  ...
                  {d.image && (
                    <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-200 w-fit self-start md:self-center">
                      ...
                      <div>
                        ...
                        <a href={`http://localhost:5000${d.image}`} target="_blank" rel="noreferrer" className="text-[10px] text-emerald-600 font-black uppercase hover:underline flex items-center gap-1 mt-0.5">
                  ...
                  <div className="p-6 bg-emerald-50/40 rounded-[24px] border border-emerald-100">
                    ...
                    {d.answerImage && (
                      <div className="flex items-center gap-3 bg-white p-3 rounded-2xl border border-emerald-100 w-fit">
                          <img 
                            src={`http://localhost:5000${d.answerImage}`} 
                          ...
                          <div>
                            ...
                            <a href={`http://localhost:5000${d.answerImage}`} target="_blank" rel="noreferrer" className="text-[10px] text-emerald-600 font-black uppercase hover:underline flex items-center gap-1 mt-0.5">
