...
const StudentDashboard = () => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 lg:space-y-8 pb-6 lg:pb-10 animate-in fade-in duration-500 bg-[#F8FAFC]">

      {/* Welcome Banner */}
      <div className="relative overflow-hidden bg-white border border-slate-100 rounded-2xl sm:rounded-[32px] p-5 sm:p-7 lg:p-10 shadow-sm">
        <div className="absolute top-0 right-0 w-24 h-24 lg:w-32 lg:h-32 bg-emerald-50 rounded-full -mr-12 -mt-12 lg:-mr-16 lg:-mt-16 opacity-50"></div>
        <div className="relative z-10">
          <h1 className="text-xl sm:text-2xl lg:text-3xl font-black text-slate-800 flex items-center gap-2 lg:gap-3">
            Welcome back, {user?.name || 'Student'}! <span className="animate-bounce">👋</span>
          </h1>
          <p className="text-slate-500 mt-1 lg:mt-2 font-medium text-sm lg:text-base max-w-md">
            {data.stats.progress > 0
              ? `You have completed ${data.stats.progress}% of your course. Keep it up!`
              : "Start your first lecture today to begin your journey!"}
          </p>
      ...
      {/* Stats - mobile mein 2x2 grid */}
      <div className="grid grid-cols-2 lg:hidden gap-3">
        <StatCard icon={<Video size={20} />} label="Total Lectures" value={data.stats.totalLectures} color="bg-emerald-50 text-[#059669]" small />
        <StatCard icon={<Clock size={20} />} label="Attendance Rate" value={`${data.stats.attendanceRate}%`} color="bg-emerald-50 text-[#059669]" small />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
        <div className="lg:col-span-2 space-y-4 sm:space-y-6 lg:space-y-8">
          ...
          {/* Main Course Progress */}
          <div className="bg-white p-5 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[32px] shadow-sm border border-slate-100">
            <h3 className="text-sm lg:text-lg font-black text-slate-800 mb-4 lg:mb-8 uppercase tracking-tight">Current Course</h3>
            <div className="flex items-center gap-4 lg:gap-6 mb-4 lg:mb-8">
              <div className="w-12 h-12 lg:w-16 lg:h-16 bg-[#059669] rounded-2xl flex items-center justify-center text-white shadow-lg shadow-emerald-200 flex-shrink-0">
                <BookOpen size={22} className="lg:hidden" />
                <BookOpen size={28} className="hidden lg:block" />
              </div>
              <div className="min-w-0">
                <h4 className="text-base lg:text-xl font-bold text-slate-800 truncate">{data.stats.enrolledCourse}</h4>
                <p className="text-xs sm:text-sm text-[#059669] font-bold uppercase tracking-wider opacity-80">
                  Status: {data.stats.totalLectures > 0 ? 'Active' : 'Pending Enrollment'}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2 lg:space-y-4">
              <div className="flex justify-between items-end">
                <span className="text-xs lg:text-sm font-bold text-slate-500">Course Completion</span>
                <span className="text-xl lg:text-2xl font-black text-[#059669]">{data.stats.progress}%</span>
              </div>
              <div className="w-full bg-slate-100 h-3 lg:h-4 rounded-full overflow-hidden">
                <div
                  className="bg-[#059669] h-full rounded-full transition-all duration-1000 shadow-[0_0_10px_rgba(5,150,105,0.3)]"
                  style={{ width: `${data.stats.progress}%` }}
                ></div>
          ...
          {/* Continue Watching */}
          <div className="bg-white p-5 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[32px] shadow-sm border border-slate-100">
            <h3 className="text-sm lg:text-lg font-black text-slate-800 mb-4 lg:mb-6 uppercase tracking-tight flex items-center gap-2">
              <PlayCircle className="text-[#059669]" size={18} /> Continue Watching
            </h3>
            ...
            {data.continueWatching && (Array.isArray(data.continueWatching) ? data.continueWatching.length > 0 : true) ? (
              <div className="space-y-3 lg:space-y-4">
                {(Array.isArray(data.continueWatching) ? data.continueWatching : [data.continueWatching]).map((video, index) => (
                  <div
                    key={video.id || index}
                    className="flex flex-col sm:flex-row gap-4 lg:gap-6 items-start sm:items-center bg-slate-50 p-3 sm:p-4 rounded-2xl border border-slate-100 hover:border-emerald-200 transition-all group"
                  >
                    {/* Thumbnail */}
                    <div
                      onClick={() => handleResumeVideo(video.videoUrl)}
                      className="relative w-full sm:w-40 lg:w-48 h-24 lg:h-28 bg-slate-200 rounded-xl overflow-hidden shadow-inner cursor-pointer flex-shrink-0"
                    >
                      <img
                        src={video.thumbnail || "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?q=80&w=400"}
                        className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
                    ...
                    {/* Text & Action */}
                    <div className="flex-1 w-full">
                      <h4 className="font-bold text-slate-800 text-sm lg:text-base mb-1 group-hover:text-[#059669] transition-colors line-clamp-1">
                        {video.title}
                      </h4>
                      <p className="text-[10px] text-slate-400 mb-3 lg:mb-4 uppercase font-black tracking-widest italic">
                        {video.moduleName || "Recent Lecture"}
                      </p>
                      <button
                        ...
                        className="px-4 py-2 lg:px-5 lg:py-2.5 bg-[#059669] text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-900 transition-all flex items-center gap-2 shadow-lg shadow-emerald-900/10 active:scale-95"
                      >
                        Resume Now <ArrowRight size={12} />
                      </button>
                    </div>
                  </div>
                ))}
              ...
              <div className="p-6 lg:p-10 border-2 border-dashed border-slate-100 rounded-[24px] text-center bg-slate-50/30">
                <Video className="mx-auto text-slate-200 mb-3" size={28} />
                <p className="text-slate-400 text-xs sm:text-sm font-medium">No recent activity. Head to 'My Lectures' to start!</p>
              </div>
            )}
          </div>
        </div>

        {/* Sidebar Stats - sirf lg pe dikhega */}
        <div className="hidden lg:flex flex-col gap-6">
          <StatCard icon={<Video size={24} />} label="Total Lectures" value={data.stats.totalLectures} color="bg-emerald-50 text-[#059669]" />
          <StatCard icon={<Clock size={24} />} label="Attendance Rate" value={`${data.stats.attendanceRate}%`} color="bg-emerald-50 text-[#059669]" />
              ...
              {data.upcomingLiveClasses.length > 0 ? (
                data.upcomingLiveClasses.map((live, idx) => (
                  <div key={idx} className="p-4 bg-emerald-50/50 rounded-2xl border border-emerald-100">
                    ...
                    <h5 className="font-bold text-slate-800 text-sm">{live.topic}</h5>
                    {live.zoomDetails?.meeting_id ? (
                      <button onClick={() => setActiveLiveLecture(live)} className="mt-3 block w-full py-2 text-center bg-[#059669] text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-900 transition-all shadow-md active:scale-95">
                        Join Zoom Class
                      </button>
                    ) : live.meetingLink ? (
                      <a href={live.meetingLink.startsWith('http') ? live.meetingLink : `https://${live.meetingLink}`} target="_blank" rel="noopener noreferrer" className="mt-3 block w-full py-2 text-center bg-white border border-emerald-200 rounded-xl text-xs font-bold text-[#059669] hover:bg-[#059669] hover:text-white transition-all shadow-sm">
                        Join via Link
                      </a>
                    ) : (
                      <button className="mt-3 block w-full py-2 text-center bg-slate-100 text-slate-400 border border-slate-200 rounded-xl text-xs font-bold cursor-not-allowed uppercase">
      ...
      {/* Upcoming Live Classes - sirf sm/md pe dikhega */}
      <div className="lg:hidden bg-white p-5 sm:p-6 rounded-2xl border border-slate-100 shadow-sm">
        <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center justify-between">
          Upcoming Live Classes <Calendar size={14} />
        </h3>
        <div className="space-y-3">
          {data.upcomingLiveClasses.length > 0 ? (
            data.upcomingLiveClasses.map((live, idx) => (
              <div key={idx} className="p-3 bg-emerald-50/50 rounded-2xl border border-emerald-100">
                <p className="text-[10px] font-bold text-[#059669] mb-1">{live.time} • {live.date}</p>
                <h5 className="font-bold text-slate-800 text-sm">{live.topic}</h5>
                {live.zoomDetails?.meeting_id ? (
                  <button onClick={() => setActiveLiveLecture(live)} className="mt-2 block w-full py-2 text-center bg-[#059669] text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-900 transition-all">
                    Join Zoom Class
                  </button>
                ) : live.meetingLink ? (
                  <a href={live.meetingLink.startsWith('http') ? live.meetingLink : `https://${live.meetingLink}`} target="_blank" rel="noopener noreferrer" className="mt-2 block w-full py-2 text-center bg-white border border-emerald-200 rounded-xl text-xs font-bold text-[#059669] hover:bg-[#059669] hover:text-white transition-all">
                    Join via Link
                  </a>
                ) : (
                  <button className="mt-2 block w-full py-2 text-center bg-slate-100 text-slate-400 rounded-xl text-xs font-bold cursor-not-allowed uppercase">
                    Link TBA
                  </button>
                )}
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
        ...
        {/* Announcements */}
        <div className="bg-white p-5 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[32px] shadow-sm border border-emerald-100">
          <div className="flex justify-between items-center mb-4 lg:mb-6">
            <h3 className="text-sm lg:text-lg font-black text-slate-800 uppercase tracking-tight flex items-center gap-2">
              <Megaphone className="text-emerald-600" size={18} /> Latest Updates
            </h3>
          ...
          <div className="space-y-3 lg:space-y-4 max-h-[300px] lg:max-h-[400px] overflow-y-auto pr-1">
            {data.announcements.length > 0 ? (
              data.announcements.map((msg, idx) => {
                const isNew = new Date(msg.createdAt) > new Date(Date.now() - 24 * 60 * 60 * 1000);
                return (
                  <div key={idx} className="flex gap-3 lg:gap-4 p-3 sm:p-5 bg-emerald-50/30 rounded-2xl transition-all border border-emerald-50 hover:border-emerald-200 hover:bg-white group shadow-sm shadow-emerald-500/5">
                    <div className="relative shrink-0">
                      <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-2xl flex items-center justify-center transition-transform group-hover:scale-110 shadow-sm ${isNew ? 'bg-emerald-600 text-white shadow-emerald-200' : 'bg-slate-100 text-slate-400'}`}>
                        <Megaphone size={18} />
                      </div>
                      {isNew && <span className="absolute -top-1 -right-1 w-3 h-3 lg:w-4 lg:h-4 bg-red-500 border-2 border-white rounded-full animate-pulse"></span>}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-1 gap-2">
                        <h5 className="font-bold text-slate-800 text-xs sm:text-sm group-hover:text-emerald-700 transition-colors truncate">
                          {msg.title}
                          {isNew && <span className="ml-1 text-[9px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full uppercase font-black">New</span>}
                        </h5>
              ...
              <div className="text-center py-10 border-2 border-dashed border-emerald-50 rounded-[32px] bg-emerald-50/20">
                <Megaphone className="mx-auto text-emerald-100 mb-3" size={36} />
                <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">No announcements today</p>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Certificates */}
        <div className="bg-slate-900 p-5 sm:p-6 lg:p-8 rounded-2xl sm:rounded-[32px] shadow-sm text-white relative overflow-hidden">
          <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-white/5 rounded-full"></div>
          <h3 className="text-sm lg:text-lg font-black text-white mb-4 lg:mb-6 uppercase tracking-tight flex items-center gap-2">
            <Award className="text-emerald-400" size={18} /> Certificates Earned
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 lg:gap-4 relative z-10">
            {data.certificates && data.certificates.length > 0 ? (
              data.certificates.map((cert, idx) => (
                <div key={idx} className="bg-white/5 p-4 lg:p-5 rounded-2xl border border-white/10 flex flex-col items-start gap-2 lg:gap-3 hover:border-emerald-500 transition-all group">
                  <div className="w-9 h-9 lg:w-10 lg:h-10 bg-emerald-500/20 rounded-xl flex items-center justify-center">
                  ...
                  <button
                    onClick={() => handleDownloadCertificate(cert.url, cert.title)}
                    className="w-full mt-1 py-2 bg-white/10 hover:bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 border border-white/5"
                  >
              ...
              <div className="col-span-2 text-center py-8 opacity-40 border-2 border-dashed border-white/10 rounded-3xl">
                <Award size={36} className="mx-auto mb-3 text-slate-500" />
                <p className="text-xs font-bold uppercase tracking-widest">Achievements Locked</p>
                <p className="text-[10px] mt-1 text-slate-400">Complete your course to earn yours.</p>
              </div>
            )}
          </div>
        </div>
      ...
      {/* Video Modal */}
      {selectedVideo && (
        <div className="fixed inset-0 bg-slate-950/95 flex items-center justify-center z-[9999] p-3 sm:p-4 backdrop-blur-sm">
          <div className="relative w-full max-w-5xl bg-black rounded-2xl sm:rounded-[32px] overflow-hidden border-4 border-white/10 shadow-2xl">
            <button
              onClick={() => setSelectedVideo("")}
              className="absolute top-3 right-3 z-[10000] bg-white/20 hover:bg-red-500 text-white p-2 rounded-full transition-all"
            >
              <X size={20} />
            </button>
            <div className="aspect-video">
            ...
            <div className="p-4 sm:p-5 bg-slate-900 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <div className="text-left">
                <p className="text-white font-bold text-sm">{data.continueWatching?.title}</p>
                <p className="text-white/40 text-[10px] uppercase font-black">Video is playing from your last session</p>
              </div>
              <div className="flex gap-2 sm:gap-3 w-full sm:w-auto">
                <button onClick={() => setSelectedVideo("")} className="flex-1 sm:flex-none px-4 py-2 rounded-xl text-white bg-white/10 font-bold hover:bg-white/20 transition-all text-xs sm:text-sm">
                  Watch Later
                </button>
                <button
                  onClick={() => handleMarkComplete(data.continueWatching.id || data.continueWatching._id)}
                  className="flex-1 sm:flex-none px-4 py-2 rounded-xl text-white bg-emerald-600 font-bold hover:bg-emerald-500 shadow-lg transition-all text-xs sm:text-sm flex items-center justify-center gap-1"
                >
                  Mark Complete ✅
                </button>
              </div>
...
const StatCard = ({ icon, label, value, color, small }) => (
  <div className={`bg-white ${small ? 'p-4 rounded-2xl' : 'p-7 rounded-[32px]'} border border-slate-100 shadow-sm flex items-center gap-3 lg:gap-5 group hover:border-[#059669]/30 transition-all hover:shadow-md`}>
    <div className={`${small ? 'w-10 h-10' : 'w-14 h-14'} ${color} rounded-2xl flex items-center justify-center transition-transform group-hover:scale-110 shadow-sm flex-shrink-0`}>
      {icon}
    </div>
    <div>
      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{label}</p>
      <h3 className={`${small ? 'text-xl' : 'text-2xl'} font-black text-slate-800`}>{value}</h3>
    </div>
  </div>
);
