  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <div className="mb-6 sm:mb-8 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[#059669] text-xs sm:text-sm font-medium">
            Upcoming live sessions for your courses
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-xl shadow-emerald-900/5 overflow-hidden">
        ...
        {/* DESKTOP TABLE (lg same) */}
        <table className="w-full text-left border-collapse hidden md:table">
          <thead>
            <tr className="bg-[#059669] text-white font-bold text-[11px] uppercase">
              <th className="px-6 py-4 text-[10px] tracking-[0.2em]">Date</th>
              <th className="px-6 py-4 text-[10px] tracking-[0.2em]">Topic</th>
              <th className="px-6 py-4 text-[10px] tracking-[0.2em] text-center">Time</th>
              <th className="px-6 py-4 text-[10px] tracking-[0.2em] text-right">Action</th>
            </tr>
          </thead>
            ...
            {schedules.length > 0 ? (
              schedules.map((lecture) => (
                <tr key={lecture._id} className="hover:bg-emerald-50/50 transition-colors">
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                      <span className="font-bold text-slate-800 capitalize tracking-tight">
                        {lecture.title}
                      </span>
                      <span className="text-[10px] text-slate-400 font-bold uppercase">
                        {lecture.course?.title}
                  ...
                  <td className="px-6 py-5 text-right">
                    {lecture.meetingLink ? (
                      <button
                        onClick={() => setActiveLecture(lecture)}
                        className="bg-slate-900 text-white px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#059669] transition-all shadow-lg flex items-center gap-2 ml-auto"
                      >
                        <Video size={14} /> Join Class
                      </button>
                    ) : (
        ...
        <div className="md:hidden divide-y">
          {schedules.length > 0 ? (
            schedules.map((lecture) => (
              <div key={lecture._id} className="p-4 space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="bg-emerald-50 text-[#059669] px-3 py-1 rounded-full text-[10px] font-black flex items-center gap-1">
                    <Clock size={12} /> {lecture.startTime || "TBA"}
                  </div>
                </div>
                ...
                <div>
                  {lecture.meetingLink ? (
                    <button
                      onClick={() => setActiveLecture(lecture)}
                      className="w-full bg-slate-900 text-white py-3 rounded-xl text-[11px] font-black uppercase tracking-widest hover:bg-[#059669] transition-all flex items-center justify-center gap-2"
                    >
                      <Video size={14} /> Join Class
                    </button>
                  ) : (
                    <p className="text-[10px] font-bold text-slate-300 italic uppercase text-center">
                      Link TBA
                    </p>
                  )}
                </div>
            ...
            <div className="px-6 py-20 text-center">
              <VideoOff size={40} className="text-[#059669] mx-auto mb-2 opacity-20" />
              <p className="font-black text-sm uppercase tracking-widest text-slate-900 opacity-30">
