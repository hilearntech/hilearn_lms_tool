...
const StudentAssignments = () => {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-6 max-w-6xl mx-auto bg-white min-h-screen">

      {/* HEADER */}
      <div className="flex flex-col sm:flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b pb-6">

        <div className="flex items-center gap-3 sm:gap-4">
          <div className="p-2 sm:p-3 bg-[#059669] rounded-xl text-white shadow-md">
        ...
        <div className="w-full sm:w-full md:w-auto bg-[#f0fdf4] border border-[#dcfce7] py-2 px-4 sm:px-6 rounded-2xl flex items-center justify-center md:justify-start gap-3 shadow-sm">
      ...
      <div className="grid grid-cols-1 gap-6">
        {assignments.length === 0 ? (
          ...
          assignments.map((assign) => {
            ...
            return (
              <div key={assign._id} className="bg-white rounded-[24px] border border-slate-200 p-4 sm:p-6 flex flex-col lg:flex-row justify-between items-stretch lg:items-center hover:border-[#059669] transition-all hover:shadow-xl group">
                ...
                <div className="flex-1 lg:pr-8 border-slate-100 lg:border-r mb-6 lg:mb-0">
                  ...
                  <h3 className="text-lg sm:text-xl font-extrabold text-slate-800 mb-2 group-hover:text-[#059669] transition-colors">
                    {assign.title}
                  </h3>

                  {/* IMAGE */}
                  {assign.image && (
                    <div className="relative mt-3 mb-4 w-full sm:w-56 h-28 rounded-xl overflow-hidden border border-slate-100 group/img cursor-pointer shadow-sm"
                      onClick={() => window.open(`https://hilearnlmstool-production.up.railway.app${assign.image}`, '_blank')}>
                ...
                {/* RIGHT */}
                <div className="w-full lg:w-[400px] flex items-center justify-center lg:pl-8">

                  {userSubmission ? (
                    <div className="w-full space-y-4">

                      <div className={`rounded-3xl p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4 border transition-all ${isGraded ? 'bg-emerald-50/40 border-emerald-100 shadow-sm' : 'bg-amber-50/50 border-amber-200 border-dashed'}`}>

                        <div className="text-center sm:min-w-[90px] sm:border-r border-slate-200 sm:pr-4">
                          <p className={`text-[10px] font-black uppercase mb-1 ${isGraded ? 'text-emerald-600' : 'text-amber-600'}`}>
                            {isGraded ? "Final Grade" : "Current Status"}
                          </p>

                          {isGraded ? (
                            <p className="text-3xl sm:text-4xl font-black text-[#059669]">
                              {userSubmission.grade}
                            </p>
                          ) : (
                            <div className="flex flex-col items-center gap-1">
                              <Timer size={22} className="text-amber-500 animate-pulse" />
                              <span className="text-[10px] font-bold text-amber-700">Pending</span>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex-1">
                          {isGraded ? (
                            ...
                            <>
                              <p className="text-sm font-black text-amber-800">Under Evaluation</p>
                    ...
                    <div className="w-full space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 px-4 py-4 rounded-2xl">
                      ...
                      <button
                        onClick={() => handleSubmit(assign._id)}
                        className="w-full py-4 bg-[#059669] text-white rounded-2xl text-sm font-black hover:bg-slate-900 transition-all flex items-center justify-center gap-2"
                      >
                        <Send size={16} /> Submit Project
                      </button>
