...
const StudentAssignments = () => {
  ...
  const fetchAssignments = async () => {
    try {
      ...
      const res = await axios.get("http://localhost:5000/api/student/my-assignments", {
  ...
  const handleSubmit = async (assignmentId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post("http://localhost:5000/api/student/submit-assignment", 
                ...
                <div className="flex-1 lg:pr-8 border-slate-100 lg:border-r mb-6 lg:mb-0">
                  ...
                  {assign.image && (
                    <div className="relative mt-3 mb-4 w-48 h-28 rounded-xl overflow-hidden border border-slate-100 group/img cursor-pointer shadow-sm"
                         onClick={() => window.open(`http://localhost:5000${assign.image}`, '_blank')}>
                      <img 
                        src={`http://localhost:5000${assign.image}`} 
                        alt="Task Preview" 
