...
const ReportManagement = () => {
  ...
  return (
    <div className="p-4 sm:p-5 md:p-6 space-y-6 md:space-y-8 bg-gray-50 min-h-screen">

      {/* Header Section */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 sm:p-5 md:p-6 rounded-3xl shadow-sm border border-gray-100">
        <div>
          <h2 className="text-xl sm:text-2xl font-black text-gray-900 tracking-tight">
            Analytics Dashboard
          </h2>
          <p className="text-gray-500 text-xs sm:text-sm font-medium">
            Data-driven insights for HiLearn Management
          </p>
        </div>

        <button
          onClick={downloadPDF}
          className="w-full md:w-auto flex items-center justify-center gap-2 px-4 sm:px-6 py-3 bg-emerald-600 text-white rounded-2xl font-bold text-sm hover:bg-emerald-700 transition-all shadow-lg active:scale-95"
        >
          <Download className="w-5 h-5" /> Export PDF Report
        </button>
      </div>
        ...
        <ChartContainer title="Batch Attendance Status (%)">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data?.batchWiseAttendance}>
              ...
              <Tooltip />
              <Bar dataKey="attendance" fill="#10b981" radius={[6, 6, 0, 0]} barSize={28} />
            </BarChart>
          </ResponsiveContainer>
        </ChartContainer>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        ...
        {/* Top Performers */}
        <div className="bg-white p-5 sm:p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm">
          ...
          <div className="space-y-3 md:space-y-4">
            {data?.topStudents?.map((student, index) => (
              <div key={index} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 sm:p-4 bg-gray-50 rounded-2xl">

                <div className="flex items-center gap-3 sm:gap-4">
                  <div className="w-9 h-9 sm:w-10 sm:h-10 bg-white shadow-sm rounded-xl flex items-center justify-center font-bold text-emerald-600">
                    {index + 1}
                  </div>
                  <div>
        ...
        <div className="bg-white p-5 sm:p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm">
          ...
          <div className="space-y-3 md:space-y-4">
            {data?.batchWiseAttendance?.filter(b => b.attendance < 75).map((batch, index) => (
              <div key={index} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-3 sm:p-4 bg-rose-50 rounded-2xl border border-rose-100">
                <div>
                  <p className="font-bold text-rose-900 text-sm sm:text-base">{batch.name}</p>
                  <p className="text-xs text-rose-600">Critical Attendance level</p>
                </div>
                <div className="px-3 py-1 bg-white rounded-xl text-rose-600 font-black text-sm">
                  {batch.attendance}%
                </div>
              </div>
            ))}
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="p-4 sm:p-5 md:p-8 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              type="text"
              placeholder="Search batch name..."
              className="w-full pl-12 pr-4 py-2 sm:py-3 bg-gray-50 border-none rounded-2xl text-sm outline-none focus:ring-2 focus:ring-emerald-500"
              onChange={(e) => setSearchTerm(e.target.value)}
            />
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left min-w-[600px]">
            <thead className="bg-gray-50 text-gray-400 text-[10px] sm:text-[11px] uppercase font-black">
              <tr>
                <th className="px-4 sm:px-6 md:px-8 py-4">Batch Details</th>
                <th className="px-4 sm:px-6 md:px-8 py-4">Status</th>
                <th className="px-4 sm:px-6 md:px-8 py-4">Attendance</th>
                <th className="px-4 sm:px-6 md:px-8 py-4 text-right">Actions</th>
              </tr>
            </thead>
                ...
                .map((batch, i) => (
                  <tr key={i} className="hover:bg-gray-50">
                    <td className="px-4 sm:px-6 md:px-8 py-4">
                      <p className="font-bold text-gray-800 text-sm sm:text-base">{batch.name}</p>
                      <p className="text-xs text-gray-400">Standard Academic Batch</p>
                    </td>
                    ...
                    <td className="px-4 sm:px-6 md:px-8 py-4">
                      <div className="flex items-center gap-2 sm:gap-4">
                        <div className="flex-1 h-2 bg-gray-100 rounded-full">
                          <div
                            className={`${batch.attendance < 75 ? 'bg-rose-500' : 'bg-emerald-500'} h-full`}
                            style={{ width: `${batch.attendance}%` }}
                          />
                        </div>
                    ...
                    <td className="px-4 sm:px-6 md:px-8 py-4 text-right">
                      <button className="p-2 hover:bg-emerald-100 rounded-lg text-emerald-600">
