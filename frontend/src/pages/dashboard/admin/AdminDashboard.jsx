...
//         const res = await axios.get("https://hilearnlmstool-production.up.railway.app/api/admin/stats", {
...
const AdminDashboard = () => {
  ...
  if (loading) return <div className="p-4 text-[#059669] font-bold">Refreshing stats...</div>;
  ...
  return (
    <div className="space-y-6">
      {/* Welcome Banner */}
      <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden w-full min-w-0">
        <div className="relative z-10">
          <h1 className="text-xl font-black text-slate-800">
            Welcome back, Admin! 👋
          </h1>
          <p className="text-slate-500 font-medium mt-1 text-sm">
            Everything looks good today. Here's your overview.
          </p>
        </div>
        <div className="absolute top-0 right-0 p-6 opacity-10">
          <Layers size={80} className="text-[#059669]" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {cards.map((card, index) => (
          <div
            key={index}
            className={`bg-white rounded-2xl border border-slate-100 p-4 hover:shadow-lg transition-all ${
              index === 4 ? "col-span-2 lg:col-span-1" : ""
            }`}
          >
            <div className="flex items-center gap-3">
              <div className={`p-3 rounded-xl ${card.bg} ${card.color} flex-shrink-0`}>
                {card.icon}
              </div>
              <div className="min-w-0">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-tight truncate">
                  {card.label}
                </p>
                <h2 className="text-xl font-black text-slate-800">{card.value}</h2>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="p-8 bg-white rounded-2xl border border-dashed border-slate-200 flex flex-col items-center justify-center text-center">
        <div className="w-14 h-14 sm:w-16 sm:h-16 bg-emerald-50 text-[#059669] rounded-full flex items-center justify-center mb-4">
          <Layers size={28} />
        </div>
        <h3 className="text-base sm:text-lg font-bold text-slate-800">Operational Analytics</h3>
        <p className="text-slate-500 text-xs sm:text-sm max-w-xs sm:max-w-sm mt-1">
          Detailed student growth and lecture engagement metrics will be available as more data is collected.
        </p>
        <button className="mt-5 sm:mt-6 px-8 sm:px-10 py-2.5 sm:py-3 bg-[#059669] text-white rounded-full font-bold text-sm shadow-lg shadow-emerald-100 active:scale-95 transition-all">
          Generate Report
        </button>
      </div>
    </div>
