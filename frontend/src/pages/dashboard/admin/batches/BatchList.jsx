...
const BatchList = () => {
  ...
  return (
    <div>
      <h1 className="text-xl sm:text-2xl font-bold text-[#059669] mb-4 sm:mb-6">
        Manage Batches
      </h1>

      {/* ADD BATCH */}
      <form onSubmit={handleAdd} className="flex flex-col md:flex-row gap-3 sm:gap-4 mb-6">
        <input
          value={name}
          onChange={(e) => setName(e.target.value)}
          placeholder="Batch Name"
          className="border p-2.5 sm:p-3 rounded w-full md:w-64 text-sm focus:border-[#059669] outline-none"
          required
        />
        ...
        <button className="bg-[#059669] hover:bg-[#047857] text-white py-2.5 sm:py-3 px-4 sm:px-6 rounded text-sm transition-colors w-full md:w-auto">
          Add Batch
        </button>
      </form>
          ...
          <tbody>
            {batches.map((b) => (
              <tr key={b._id} className="border-t">
                ...
                <td>
                  <span
                    className={`px-2 sm:px-3 py-1 rounded-full text-[10px] sm:text-xs ${b.isActive
                        ? "bg-emerald-100 text-emerald-700"
                        : "bg-red-100 text-red-700"
                      }`}
                  >
                ...
                <td className="flex flex-wrap md:flex-nowrap gap-2 sm:gap-3 justify-center py-3 items-center">
                  <button
                    onClick={() => {
                      setSelectedBatchId(b._id);
                      setShowStudentModal(true);
                    ...
                    className="text-indigo-600 font-semibold text-[10px] sm:text-xs border border-indigo-200 px-2 py-1 rounded-lg hover:bg-indigo-50"
                  >
                    + Add Student
                  </button>
                  ...
                  <button
                    onClick={() => setEditBatch(b)}
                    className="text-amber-600 font-semibold text-[10px] sm:text-xs"
                  >
                    Edit
                  </button>
                  ...
                  <div className="flex items-center">
                    <button
                      type="button"
                      onClick={() => handleToggleStatus(b)}
                      className={`relative inline-flex h-6 sm:h-7 w-12 sm:w-14 items-center px-1 rounded-full 
                  transition-all duration-300
                  ${b.isActive
                          ? "bg-[#059669] justify-end shadow-[0_0_8px_rgba(5,150,105,0.4)]"
                          : "bg-gray-300 justify-start"
                    ...
                    >
                      <span className="inline-block h-5 sm:h-6 w-5 sm:w-6 rounded-full bg-white shadow-md" />
                    </button>
                  </div>

                  <button
                    onClick={() => deleteBatch(b._id).then(loadBatches)}
                    className="text-red-600 font-semibold text-[10px] sm:text-xs"
                  >
                    Delete
                  </button>
                  ...
                  <button
                    ...
                    className="text-blue-600 font-bold text-[10px] sm:text-xs flex items-center gap-1"
                  >
                    View
                  </button>
                </td>
      ...
      {showStudentModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-3 sm:p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[95%] sm:max-w-md overflow-hidden flex flex-col max-h-[90vh]">

            <div className="p-3 sm:p-4 border-b bg-emerald-50 flex justify-between items-center">
              <h2 className="text-base sm:text-xl font-bold text-emerald-800">
                Add Students to Batch
              </h2>
              <button onClick={() => setShowStudentModal(false)} className="text-gray-500 hover:text-red-500 text-xl sm:text-2xl">
                &times;
              </button>
            </div>
            ...
            <div className="p-3 sm:p-4">
              <input
                type="text"
                placeholder="Search by name or email..."
                className="w-full pl-8 sm:pl-10 pr-3 sm:pr-4 py-2 border rounded-lg text-xs sm:text-sm focus:ring-2 focus:ring-emerald-500 outline-none"
                onChange={(e) => setSearchTerm(e.target.value)}
              />
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {allStudents.map((student) => (
                <label key={student._id} className="flex items-center p-2.5 sm:p-3 hover:bg-emerald-50 rounded-xl cursor-pointer border-b">
                  <input
                    type="checkbox"
                    className="w-4 sm:w-5 h-4 sm:h-5 accent-emerald-600"
                    checked={checkedStudents.includes(student._id)}
                    onChange={(e) => {
                      if (e.target.checked) {
                        setCheckedStudents([...checkedStudents, student._id]);
                  ...
                  <div className="ml-3 sm:ml-4">
                    <p className="font-semibold text-gray-800 text-xs sm:text-sm">{student.name}</p>
            ...
            <div className="p-3 sm:p-4 border-t bg-gray-50 flex flex-col sm:flex-row gap-2 sm:gap-3">
              <button
                onClick={() => setShowStudentModal(false)}
                className="w-full py-2 text-gray-600 font-semibold border rounded-lg text-xs sm:text-sm"
              >
                Cancel
              </button>
              <button
                onClick={handleAssignStudents}
                disabled={checkedStudents.length === 0}
                className="w-full py-2 text-white font-semibold rounded-lg text-xs sm:text-sm bg-emerald-600"
              >
                Assign ({checkedStudents.length})
              </button>
            </div>
      ...
      {showViewModal && selectedBatch && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-3 sm:p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[95%] sm:max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">

            <div className="p-4 sm:p-5 border-b bg-blue-50 flex justify-between items-center">
              <div>
                <h2 className="text-lg sm:text-2xl font-bold text-blue-900">{selectedBatch.name}</h2>
                <p className="text-xs sm:text-sm text-blue-600">{selectedBatch.course?.title}</p>
              </div>
              <button onClick={() => setShowViewModal(false)} className="text-2xl sm:text-3xl text-gray-400 hover:text-red-500">
                &times;
              </button>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs sm:text-sm min-w-[500px]">
                  <thead className="bg-gray-100">
                    <tr>
                      <th className="p-2 sm:p-3 text-left">Student Name</th>
            ...
            <div className="p-3 sm:p-4 border-t text-right">
              <button className="px-4 sm:px-6 py-2 bg-gray-800 text-white rounded-lg text-xs sm:text-sm">
                Close
              </button>
            </div>
          </div>
