...
//             const res = await axios.get("https://hilearnlmstool-production.up.railway.app/api/admin/students-list", {
...
//         try {
//             const token = localStorage.getItem("token");
//             await axios.post("https://hilearnlmstool-production.up.railway.app/api/admin/issue-certificate", {
    ...
    return (
        <div className="p-4 sm:p-6 md:p-8 bg-slate-50 min-h-screen">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
                ...
                <div>
                    <h1 className="text-2xl sm:text-3xl font-bold text-[#059669]">
                        Certificate Console
                    </h1>
                    <p className="text-slate-400 text-xs sm:text-sm font-bold">
                        Manage and issue credentials to students
                    </p>
                </div>
                ...
                <div className="relative w-full sm:w-64">
                    ...
                    <input
                        type="text"
                        placeholder="Search student..."
                        className="pl-10 pr-4 py-2 w-full rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm"
                        onChange={(e) => setSearchTerm(e.target.value)}
                    />
                </div>
            </div>
            ...
            {/* Table */}
            <div className="bg-white rounded-[24px] shadow-sm overflow-x-auto border border-slate-100">
                ...
                <table className="min-w-[700px] w-full text-left border-collapse text-sm sm:text-base">
                    ...
                    <thead>
                        <tr className="bg-slate-50 border-b border-slate-100 text-[10px] sm:text-[11px] uppercase tracking-wider text-slate-500">
                            <th className="px-4 sm:px-6 py-3 sm:py-4 font-bold">Student</th>
                            <th className="px-4 sm:px-6 py-3 sm:py-4 font-bold">Enrolled Course</th>
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {filteredStudents.map(student => (
                            <tr key={student._id} className="hover:bg-slate-50/50 transition-all">
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                        {student.enrolledCourses && student.enrolledCourses.length > 0 ? (
                                            student.enrolledCourses.map((course, i) => (
                                                <span
                                                    key={i}
                                                    className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded-md text-[9px] sm:text-[10px] font-bold border border-slate-200"
                                                >
                                                    {course.title}
                                                </span>
                                            ))
                                ...
                                <td className="px-4 sm:px-6 py-3 sm:py-4">
                                    {student.certificates?.length > 0 ? (
                                        <span className="flex items-center gap-1 text-emerald-600 font-bold text-[10px] sm:text-xs bg-emerald-50 px-2 py-1 rounded-full w-fit">
                                            <CheckCircle size={12} /> Issued
                                        </span>
                                    ) : (
                                        <span className="text-amber-500 font-bold text-[10px] sm:text-xs bg-amber-50 px-2 py-1 rounded-full">
                                            Pending
                                        </span>
                                    )}
                                </td>
                                ...
                                <td className="px-4 sm:px-6 py-3 sm:py-4">
                                    <button
                                        onClick={() => { setSelectedStudent(student); setIsModalOpen(true); }}
                                        className="flex items-center gap-2 bg-slate-900 text-white px-3 sm:px-4 py-2 rounded-xl text-[10px] sm:text-xs font-bold hover:bg-emerald-600 transition-all shadow-md shadow-slate-200"
                                    >
                                        <Upload size={14} />
                                        {student.certificates?.length > 0 ? "Re-issue" : "Issue Cert"}
                                    </button>
                                </td>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                    ...
                    <div className="bg-white rounded-[24px] sm:rounded-[32px] p-5 sm:p-8 max-w-md w-full shadow-2xl">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    className="w-full p-3 sm:p-4 mt-1 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:border-emerald-500 transition-all text-sm"
                                    placeholder="e.g. Full Stack Web Development"
                                    onChange={(e) => setCertData({ ...certData, title: e.target.value })}
                                />
                            </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 mt-6 sm:mt-8">
                            ...
                            <button
                                ...
                                className="w-full sm:flex-1 py-3 sm:py-4 text-xs sm:text-sm font-bold text-slate-400 hover:text-slate-600 transition-all"
                            >
                                Cancel
                            </button>
                            ...
                            <button
                                ...
                                className="w-full sm:flex-[2] bg-emerald-600 text-white py-3 sm:py-4 rounded-2xl font-bold text-xs sm:text-sm hover:bg-slate-900 transition-all shadow-lg shadow-emerald-100"
                            >
                                Confirm & Issue
                            </button>
