...
const AddEditCourse = ({ onCoursesChange, editData, clearEdit }) => {
  ...
  return (
    <div className={wrapperClass}>
  <div className={`${containerClass} p-4 sm:p-6 md:p-0`}>

    {/* Header */}
    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6 sm:mb-8">
      
      <div>
        <h2 className="text-xl sm:text-2xl font-bold text-[#059669]">
          {editData ? "Update Course" : "Add New Course"}
        </h2>
      ...
      {editData && (
        <button
          onClick={clearEdit}
          className="self-end sm:self-auto p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-full transition-all"
        >
    ...
    <form
      ...
      className="grid grid-cols-1 md:grid-cols-2 gap-x-4 sm:gap-x-6 gap-y-4 sm:gap-y-5"
    >

      {/* Title */}
      <div className="md:col-span-2 space-y-1.5 sm:space-y-2">
        <label className="text-[10px] sm:text-xs font-bold text-slate-600 uppercase tracking-widest flex items-center gap-2">
          <BookOpen size={14} className="text-[#059669]" /> Course Title
        </label>
        ...
        <input
          ...
          placeholder="e.g. Full Stack Web Development"
          required
          className="w-full bg-slate-50 border border-slate-200 text-slate-800 p-2.5 sm:p-3.5 rounded-xl focus:ring-2 focus:ring-[#059669]/10 focus:border-[#059669] outline-none transition-all font-medium text-sm"
        />
      </div>
      ...
      <div className="space-y-1.5 sm:space-y-2">
        ...
        <select
          ...
          onChange={(e) => setForm({ ...form, category: e.target.value })}
          required
          className="w-full bg-slate-50 border border-slate-200 text-slate-800 p-2.5 sm:p-3.5 rounded-xl outline-none text-sm"
        >
          <option value="general">General</option>
          <option value="frontend">Frontend Development</option>
          <option value="backend">Backend Development</option>
      ...
      <div className="space-y-1.5 sm:space-y-2">
        ...
        <input
          ...
          placeholder="999"
          required
          className="w-full bg-slate-50 border border-slate-200 p-2.5 sm:p-3.5 rounded-xl outline-none text-sm"
        />
      </div>

      {/* Rating */}
      <div className="space-y-1.5 sm:space-y-2">
        <label className="text-[10px] sm:text-xs font-bold text-slate-600 uppercase flex items-center gap-2">
      ...
      <div className="space-y-1.5 sm:space-y-2">
        <label className="text-[10px] sm:text-xs font-bold text-slate-600 uppercase flex items-center gap-2">
          <Users size={14} className="text-[#059669]" /> Student Count
        </label>
      ...
      <div className="md:col-span-2 space-y-1.5 sm:space-y-2">
        <label className="text-[10px] sm:text-xs font-bold text-slate-600 uppercase flex items-center gap-2">
          <CheckCircle size={14} className="text-[#059669]" /> Key Features
        </label>
      ...
      <div className="space-y-1.5 sm:space-y-2">
        ...
        <select
          className="w-full bg-slate-100 border border-slate-200 text-slate-400 p-2.5 sm:p-3.5 rounded-xl outline-none text-sm cursor-not-allowed"
          disabled
        >
          <option>Standard Access</option>
        </select>
      ...
      <div className="md:col-span-2 space-y-1.5 sm:space-y-2">
        ...
        <textarea
          ...
          onChange={(e) => setForm({ ...form, description: e.target.value })}
          placeholder="Briefly explain what this course covers..."
          className="w-full bg-slate-50 border border-slate-200 p-2.5 sm:p-3.5 rounded-xl outline-none h-28 sm:h-32 resize-none text-sm"
        />
      </div>
      ...
      <button
        type="submit"
        disabled={loading}
        className="md:col-span-2 bg-[#059669] hover:bg-[#047857] text-white font-bold py-3 sm:py-4 rounded-xl flex items-center justify-center gap-2 sm:gap-3 text-sm transition-all mt-2"
      >
        {loading ? (
          <span className="animate-pulse text-xs sm:text-sm font-bold uppercase tracking-widest">
            Saving Changes...
          </span>
        ) : (
          <>
