...
import { useEffect, useState } from "react";
import { Clock, Users, Star, CheckCircle2, Search, Plus, X, Trash2, Edit3 } from "lucide-react";
import AddEditCourse from "./AddEditCourse";
    ...
    <div className="w-full font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4 border-b border-gray-100 pb-4">
        <div>
          ...
          <p className="text-gray-500 text-sm font-medium mt-1">Manage your professional catalog</p>
        </div>

        <button
          onClick={() => { setShowForm(!showForm); if (showForm) setEditData(null); }}
          className={`w-full md:w-auto flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-bold transition-all shadow-sm text-sm ${
            showForm
              ? "bg-gray-100 text-gray-600 hover:bg-gray-200"
              : "bg-emerald-600 text-white hover:bg-emerald-700"
          }`}
      ...
      {/* Form */}
      {showForm && (
        <div className="mb-6 bg-gray-50 p-4 rounded-3xl border-2 border-dashed border-gray-200">
          <AddEditCourse
            onCoursesChange={() => { fetchCourses(); setShowForm(false); }}
            editData={editData}
            clearEdit={() => { setEditData(null); setShowForm(false); }}
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="w-full pl-10 pr-4 py-3 bg-gray-50 border-2 border-gray-200 rounded-2xl outline-none focus:border-emerald-600 focus:bg-white transition-all text-gray-700 text-sm"
        />
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredCourses.map((course) => {
            ...
            return (
              <div
                key={course._id}
                className="bg-white border-2 border-gray-200 rounded-3xl overflow-hidden hover:border-emerald-600 transition-all flex flex-col shadow-sm hover:shadow-md"
              >
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  {/* Stats */}
                  <div className="flex flex-wrap items-center gap-4 text-gray-500 text-xs font-semibold mb-4 pb-4 border-b border-gray-100">
                    <span className="flex items-center gap-2"><Clock size={16} /> {course.duration || "6 Months"}</span>
                    <span className="flex items-center gap-2"><Users size={16} /> {course.students || "2,500+"}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-1 bg-amber-50 px-2 py-1 rounded-xl text-amber-600 font-bold border border-amber-100 text-sm">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    <button
                      onClick={() => handleEdit(course)}
                      className="w-full py-3 bg-emerald-600 text-white font-bold rounded-2xl hover:bg-emerald-700 transition-colors flex items-center justify-center gap-2 text-sm"
                    >
                      <Edit3 size={16} /> Edit Course
                    </button>
                    <button
                      onClick={() => deleteCourse(course._id)}
                      className="w-full py-3 border-2 border-gray-200 text-gray-500 font-bold rounded-2xl hover:border-rose-200 hover:text-rose-500 hover:bg-rose-50 transition-all flex items-center justify-center gap-2 text-sm"
                    >
                      <Trash2 size={16} /> Delete From Catalog
                    </button>
                  </div>
