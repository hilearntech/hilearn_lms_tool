...
//         const response = await axios.delete(`https://hilearnlmstool-production.up.railway.app/api/admin/contact/delete/${id}`, {
...
//       try {
//         const token = localStorage.getItem("token");
//         const response = await axios.get("https://hilearnlmstool-production.up.railway.app/api/admin/contact/leads", {
//           headers: { Authorization: `Bearer ${token}` }
//         });
//         if (response.data.success) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
        ...
        <div className="bg-emerald-100 text-[#059669] px-3 sm:px-4 py-1.5 sm:py-2 rounded-2xl text-xs sm:text-sm font-bold shadow-sm w-fit">
          {leads.length} Total Leads
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl sm:rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left min-w-[750px] md:min-w-full">
            ...
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <th className="px-4 sm:px-6 py-3 sm:py-5 text-[10px] sm:text-[11px] font-bold uppercase tracking-widest text-slate-400">
                  Student
                </th>
                <th className="px-4 sm:px-6 py-3 sm:py-5 text-[10px] sm:text-[11px] font-bold uppercase tracking-widest text-slate-400">
                  Course
                ...
                <th className="px-4 sm:px-6 py-3 sm:py-5 text-[10px] sm:text-[11px] font-bold uppercase tracking-widest text-slate-400 text-right">
                  Actions
                </th>
              </tr>
            </thead>
            ...
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                <tr>
                  <td colSpan="4" className="py-14 sm:py-20 text-center text-xs sm:text-sm text-slate-400 animate-pulse">
                    Fetching enquiries...
                  </td>
                </tr>
              ) : leads.length === 0 ? (
                <tr>
                  <td colSpan="4" className="py-14 sm:py-20 text-center text-xs sm:text-sm text-slate-400">
                    No inquiry has come yet.
                  </td>
                </tr>
              ) : (
                leads.map((lead) => (
                  <tr key={lead._id} className="hover:bg-emerald-50/20 transition-all group">
                    ...
                    <td className="px-4 sm:px-6 py-4 sm:py-5">
                      ...
                      <div className="flex flex-col gap-1 mt-1">
                        ...
                        {lead.rating > 0 && (
                          <span className="flex items-center gap-1 text-[10px] sm:text-xs font-semibold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-md w-fit mt-1">
                            <Star size={12} fill="currentColor" /> {lead.rating} Stars
                          </span>
                        )}
                      </div>
                    ...
                    {/* COURSE */}
                    <td className="px-4 sm:px-6 py-4 sm:py-5">
                      <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1 rounded-full bg-emerald-50 text-[#059669] text-[10px] sm:text-xs font-bold">
                        <BookOpen size={12} /> {lead.course}
                      </div>
                    </td>

                    {/* MESSAGE */}
                    <td className="px-4 sm:px-6 py-4 sm:py-5">
                      <div className="flex items-start gap-2 max-w-[200px] sm:max-w-xs">
                        <MessageSquareText size={14} className="text-slate-300 mt-1 flex-shrink-0" />
                        <p className="text-xs sm:text-sm text-slate-600 leading-relaxed italic line-clamp-3">
                          "{lead.message}"
                        </p>
                      </div>
                    </td>
                    ...
                    {/* DATE */}
                    <td className="px-4 sm:px-6 py-4 sm:py-5">
                      <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-slate-400 font-medium">
                        <Calendar size={12} />
                    ...
                    <td className="px-4 sm:px-6 py-4 sm:py-5 text-right">
                      <button
                        onClick={() => handleDelete(lead._id)}
                        className="p-1.5 sm:p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all sm:opacity-0 sm:group-hover:opacity-100"
                      >
                        <Trash2 size={16} />
                      </button>
                    </td>
