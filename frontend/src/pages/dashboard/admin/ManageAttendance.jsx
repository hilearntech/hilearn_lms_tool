...
//       const resLectures = await axios.get("https://hilearnlmstool-production.up.railway.app/api/lectures", config);
...
//         const res = await axios.get(`https://hilearnlmstool-production.up.railway.app/api/attendance/filtered-students/${selectedLecture}`, config);
  ...
  return (
    <div className="p-4 sm:p-6 bg-white min-h-screen">
      ...
      <div className="mb-4 sm:mb-6">
        <h2 className="text-2xl sm:text-3xl font-bold text-[#059669] mb-1">
          Manage Attendance
        </h2>
        <p className="text-slate-400 text-[10px] sm:text-xs font-bold uppercase tracking-widest mt-1">
          Filtered by Enrollment
        </p>
      </div>

      {/* Controls */}
      <div className="bg-slate-50 p-4 sm:p-5 rounded-xl border border-slate-200 mb-6 sm:mb-8">

        <div className="flex flex-col md:flex-row items-stretch md:items-end gap-4">

          <div className="flex-1 w-full">
            <label className="text-[10px] sm:text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2 block">
          ...
          <div className="flex gap-2 w-full md:w-auto">
            ...
            <button
              onClick={fetchInitialData}
              className="p-2.5 sm:p-3 bg-white border border-slate-300 text-slate-500 hover:text-[#059669] rounded-lg transition-all"
            >
              <RefreshCw size={18} className={fetching ? "animate-spin" : ""} />
            </button>
            ...
            <button
              onClick={saveAttendance}
              disabled={loading || !selectedLecture || students.length === 0}
              className={`flex-1 px-4 sm:px-8 py-2.5 rounded-lg flex items-center justify-center gap-2 font-bold text-xs sm:text-sm transition-all ${!selectedLecture
                  ? "bg-slate-200 text-slate-400 cursor-not-allowed"
                  : "bg-[#059669] text-white hover:bg-[#047857] shadow-lg shadow-emerald-100"
                }`}
            >
      ...
      {/* Table */}
      <div className={`overflow-x-auto rounded-xl border border-slate-200 shadow-sm ${!selectedLecture ? "opacity-40 grayscale pointer-events-none" : "opacity-100"
        }`}>
        ...
        <table className="min-w-[600px] w-full text-left border-collapse text-sm sm:text-base">
          ...
          <thead className="bg-[#059669] text-white">
            <tr>
              <th className="px-4 sm:px-6 py-3 sm:py-4 font-bold text-xs sm:text-sm uppercase tracking-wider">
                Student Name
              </th>
              <th className="px-4 sm:px-6 py-3 sm:py-4 font-bold text-xs sm:text-sm uppercase tracking-wider text-center">
                Current Status
              </th>
              <th className="px-4 sm:px-6 py-3 sm:py-4 font-bold text-xs sm:text-sm uppercase tracking-wider text-right">
                Actions
              </th>
            </tr>
          </thead>
          ...
          <tbody className="bg-white divide-y divide-slate-100">
            ...
            {filtering ? (
              <tr>
                <td colSpan="3" className="px-4 sm:px-6 py-8 sm:py-10 text-center">
                  <div className="flex flex-col items-center gap-2">
                    <Loader2 className="animate-spin text-emerald-500" size={24} />
              ...
              students.map((student) => (
                ...
                <tr key={student._id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 sm:px-6 py-3 sm:py-4 text-slate-700 font-semibold capitalize whitespace-nowrap">
                    {student.name}
                  </td>

                  <td className="px-4 sm:px-6 py-3 sm:py-4 text-center">
                    <span className={`text-[9px] sm:text-[10px] font-black px-2 sm:px-3 py-1 rounded-full uppercase ${attendance[student._id] === "present" ? "bg-emerald-100 text-emerald-600" :
                        attendance[student._id] === "absent" ? "bg-red-100 text-red-600" : "bg-slate-100 text-slate-400"
                      }`}>
                      {attendance[student._id] || "pending"}
                    </span>
                  ...
                  <td className="px-4 sm:px-6 py-3 sm:py-4 text-right">
                    <div className="flex flex-wrap justify-end gap-2">
                      ...
                      <button
                        onClick={() => handleStatusChange(student._id, "present")}
                        className={`px-3 sm:px-4 py-1.5 rounded-lg text-[9px] sm:text-[10px] font-black transition-all border ${attendance[student._id] === "present"
                            ? "bg-emerald-500 text-white border-emerald-500 shadow-md"
                            : "bg-white text-slate-400 border-slate-200 hover:border-emerald-500 hover:text-emerald-500"
                          }`}
                      >
                      ...
                      <button
                        onClick={() => handleStatusChange(student._id, "absent")}
                        className={`px-3 sm:px-4 py-1.5 rounded-lg text-[9px] sm:text-[10px] font-black transition-all border ${attendance[student._id] === "absent"
                            ? "bg-red-500 text-white border-red-500 shadow-md"
                            : "bg-white text-slate-400 border-slate-200 hover:border-red-500 hover:text-red-500"
                          }`}
                      >
              ...
              <tr>
                <td colSpan="3" className="px-4 sm:px-6 py-8 sm:py-10 text-center text-slate-400 font-medium text-sm">
                  {selectedLecture ? "No students found who bought this course." : "Select a lecture to view students."}
                </td>
              </tr>
            )}
