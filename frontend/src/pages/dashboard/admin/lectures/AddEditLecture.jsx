...
const AddEditLecture = ({ refreshLectures, editData, clearEdit }) => {
  ...
  return (
    <div className="bg-white rounded-2xl p-4 sm:p-6 md:p-8 border border-slate-200 shadow-sm mb-6 sm:mb-8">

      {/* Header */}
      <div className="flex items-start sm:items-center gap-3 mb-6 sm:mb-8 pb-4 border-b border-slate-100">
        <div className="p-2 bg-[#059669] rounded-lg text-white">
          <LayoutGrid size={18} className="sm:w-5 sm:h-5" />
        </div>
        <div>
          <h2 className="text-lg sm:text-xl font-bold text-slate-800">
            {editData ? "Update Lecture" : "Create New Lecture"}
          </h2>
      ...
      <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">

        {/* Lecture Type */}
        <div className="md:col-span-2">
          <label className="text-[10px] sm:text-[11px] font-bold text-slate-500 uppercase mb-1 sm:mb-2 block">
            Lecture Format
          </label>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
            {['video', 'live', 'article', 'quiz'].map((type) => (
              <button
                ...
                className={`p-2.5 sm:p-3 rounded-xl border font-bold text-[10px] sm:text-xs uppercase transition-all ${form.lectureType === type
        ...
        {!isMentor && (
          <div className="md:col-span-2">
            <label className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold text-slate-500 uppercase mb-1 sm:mb-2">
              <User size={12} /> Assign Mentor/Faculty
            </label>
            <select
              value={form.mentor}
              onChange={(e) => setForm({ ...form, mentor: e.target.value })}
              required
              className="w-full bg-slate-50 border border-slate-200 p-2.5 sm:p-3.5 rounded-xl outline-none text-xs sm:text-sm"
            >
              <option value="">Choose Mentor...</option>
              {mentors.map((m) => (
        ...
        {form.lectureType === 'video' && (
          <>
            <div>
              <label className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold text-slate-500 uppercase mb-1 sm:mb-2">
                <VideoIcon size={12} /> Bunny Video ID
              </label>
              <input
                value={form.videoID}
                onChange={(e) => setForm({ ...form, videoID: e.target.value })}
                placeholder="e.g. d6413e14-..."
                className="w-full bg-slate-50 border border-slate-200 p-2.5 sm:p-3.5 rounded-xl outline-none text-xs sm:text-sm"
              />
            </div>
            <div>
              <label className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold text-slate-500 uppercase mb-1 sm:mb-2">
                <Layers size={12} /> Library ID
              </label>
              <input
        ...
        {form.lectureType === 'live' && (
          <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
            <input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="p-2.5 sm:p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm" />
            <input value={form.startTime} onChange={(e) => setForm({ ...form, startTime: e.target.value })} placeholder="10:30 AM" className="p-2.5 sm:p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm" />
            <input className="md:col-span-2 p-2.5 sm:p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm" value={form.meetingLink} onChange={(e) => setForm({ ...form, meetingLink: e.target.value })} placeholder="Meeting Link" />
          </div>
        )}
        ...
        <div className="md:col-span-2">
          <textarea
            ...
            rows="3"
            className="w-full bg-slate-50 border border-slate-200 p-2.5 sm:p-3.5 rounded-xl outline-none text-xs sm:text-sm resize-none"
          />
        </div>
        ...
        {/* Checkbox */}
        <div className="md:col-span-2 flex items-start sm:items-center gap-3 p-3 sm:p-4 bg-emerald-50 rounded-xl border border-emerald-100 mb-2">
          <input
            type="checkbox"
            id="isPreviewFree"
            checked={form.isPreviewFree}
            onChange={(e) => setForm({ ...form, isPreviewFree: e.target.checked })}
            className="w-4 h-4 sm:w-5 sm:h-5 accent-[#059669] cursor-pointer mt-1 sm:mt-0"
          />
          <label htmlFor="isPreviewFree" className="cursor-pointer">
            <span className="block text-xs sm:text-sm font-bold text-emerald-900">
              Mark as Demo Video
            </span>
            <span className="block text-[10px] sm:text-[11px] text-emerald-600 font-medium">
              If checked, this lecture will be available for free preview.
            </span>
          </label>
        </div>
        ...
        <button
          type="submit"
          disabled={loading}
          className="md:col-span-2 bg-[#059669] text-white font-bold py-3 sm:py-4 text-xs sm:text-sm rounded-xl shadow-lg active:scale-95 transition-all"
        >
          {loading ? "SAVING..." : editData ? "UPDATE LECTURE" : "PUBLISH LECTURE"}
        </button>
      </form>
