...
const LectureList = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-5 md:p-8 bg-slate-50 min-h-screen font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 sm:mb-8 gap-3 sm:gap-4">
        ...
        <button
          ...
          className={`w-full sm:w-auto flex items-center justify-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl font-bold text-xs sm:text-sm transition-all active:scale-95 shadow-sm ${isFormOpen
              ? "bg-white border border-slate-200 text-slate-600"
              : "bg-[#059669] text-white hover:bg-[#047857]"
            }`}
        >
          {isFormOpen
            ? <><X size={14} className="sm:w-4 sm:h-4" /> Close</>
            : <><Plus size={14} className="sm:w-4 sm:h-4" /> New Lecture</>}
        </button>
      </div>

      {/* Form */}
      {isFormOpen && (
        <div className="mb-8 sm:mb-10 animate-in fade-in slide-in-from-top-4 duration-300">
          <AddEditLecture
            refreshLectures={fetchLectures}
            editData={editData}
            clearEdit={() => { setIsFormOpen(false); setEditData(null); }}
      ...
      <div className="flex flex-col md:flex-row gap-3 sm:gap-4 mb-6 sm:mb-8">
        ...
        <div className="relative flex-grow">
          ...
          <input
            ...
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full pl-9 sm:pl-10 pr-3 sm:pr-4 py-2.5 bg-white border border-slate-200 rounded-xl outline-none focus:border-[#059669] text-xs sm:text-sm"
          />
        </div>
        ...
        <select
          value={filterValue}
          onChange={(e) => setFilterValue(e.target.value)}
          className="w-full md:w-auto bg-white border border-slate-200 px-3 sm:px-4 py-2.5 rounded-xl outline-none text-xs sm:text-sm font-semibold text-slate-600 cursor-pointer focus:border-[#059669]"
        >
          <option value="">All Courses</option>
          {uniqueCourses.map((c) => (
            <option key={c._id} value={c._id}>{c.title}</option>
      ...
      {loading ? (
        <div className="flex justify-center py-16 sm:py-20">
          <div className="w-8 h-8 sm:w-10 sm:h-10 border-4 border-[#059669] border-t-transparent rounded-full animate-spin"></div>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {filteredLectures.map((l) => (
            <div
              key={l._id}
              className="group bg-white rounded-2xl sm:rounded-3xl border border-slate-100 p-4 sm:p-5 shadow-sm hover:shadow-lg transition-all flex flex-col h-full"
            >

              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl flex items-center justify-center mb-3 sm:mb-4 bg-emerald-50 border border-emerald-100">
                {getLectureIcon(l.lectureType)}
              </div>

              <div className="flex-grow">
                <h3 className="text-base sm:text-lg font-bold text-slate-800 mb-1 sm:mb-2 group-hover:text-[#059669]">
                  {l.title}
                </h3>

                <p className="text-slate-500 text-[12px] sm:text-[13px] mb-3 sm:mb-4 line-clamp-2">
                  {l.description || "Course module focusing on core concepts and practical implementation."}
                </p>

                <div className="flex flex-wrap items-center gap-2 text-[10px] font-bold uppercase mb-4 sm:mb-6">
                  <div className="flex items-center gap-1 px-2 py-1 bg-slate-50 rounded-lg">
                    <Clock size={10} /> {l.duration} Mins
                  </div>
                  <div className="flex items-center gap-1 px-2 py-1 bg-slate-50 rounded-lg">
                    <Layers size={10} /> {l.lectureType || 'video'}
                  </div>
                </div>
              </div>

              {/* Actions */}
              <div className="space-y-2 mt-auto pt-3 sm:pt-4 border-t">

                {l.lectureType === 'video' && (l.videoID || l.videoUrl) && (
                  <button
                    onClick={() => setSelectedVideo(`https://iframe.mediadelivery.net/embed/${l.libraryID || '592909'}/${l.videoID || l.videoUrl}?autoplay=true`)}
                    className="w-full py-2 bg-[#059669] text-white rounded-xl text-[10px] sm:text-[11px] font-bold flex items-center justify-center gap-2"
                  >
                ...
                {l.lectureType === 'live' && l.meetingLink && (
                  <button
                    onClick={() => window.open(l.meetingLink, "_blank")}
                    className="w-full py-2 bg-rose-600 text-white rounded-xl text-[10px] sm:text-[11px] font-bold flex items-center justify-center gap-2"
                  >
                    <Radio size={12} /> Join Live Class
                  </button>
                )}

                {l.lectureType === 'article' && (
                  <button className="w-full py-2 bg-amber-500 text-white rounded-xl text-[10px] sm:text-[11px] font-bold flex items-center justify-center gap-2">
                    <FileText size={12} /> Read Article
                  </button>
                )}

                <button
                  onClick={() => setSelectedLecture(l)}
                  className="w-full py-2 bg-slate-800 text-white rounded-xl text-[10px] sm:text-[11px] font-bold flex items-center justify-center gap-2"
                >
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => handleEdit(l)}
                    className="flex-1 py-2 border text-[10px] rounded-lg font-bold flex items-center justify-center gap-1"
                  >
                    <Edit3 size={10} /> Edit
                  </button>
                  ...
                  <button
                    onClick={() => deleteLecture(l._id)}
                    className="flex-1 py-2 border text-[10px] rounded-lg text-rose-500 font-bold flex items-center justify-center gap-1"
                  >
      ...
      {/* Empty */}
      {!loading && filteredLectures.length === 0 && (
        <div className="text-center py-16 sm:py-20 bg-white rounded-2xl sm:rounded-3xl border border-dashed">
          <Layers className="text-slate-200 mx-auto mb-3" size={36} />
          <p className="text-slate-400 text-sm">No lectures found.</p>
        </div>
      )}
      ...
      {selectedVideo && (
        <div className="fixed inset-0 bg-black/90 z-[3000] flex items-center justify-center p-3 sm:p-4">
          <div className="relative w-full max-w-4xl aspect-video">
            <button
              onClick={() => setSelectedVideo("")}
              className="absolute -top-10 sm:-top-12 right-0 text-white text-xs sm:text-sm flex items-center gap-2"
            >
              <X size={20} /> CLOSE
            </button>
            <iframe
