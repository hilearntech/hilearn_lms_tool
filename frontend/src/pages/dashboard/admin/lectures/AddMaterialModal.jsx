...
export default function AddMaterialModal({ lecture, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex justify-center items-center z-50 p-3 sm:p-4">
      <div className="bg-white rounded-2xl w-full max-w-[95%] sm:max-w-md shadow-2xl border border-slate-100 overflow-hidden">
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-5">
          <div>
            ...
            <input
              placeholder="e.g. Weekly Notes PDF"
              className="w-full bg-slate-50 border border-slate-200 p-2.5 sm:p-3 rounded-xl outline-none focus:border-[#059669] text-xs sm:text-sm"
              onChange={(e) => setTitle(e.target.value)}
            />
          </div>
          ...
          <div>
            ...
            <select
              className="w-full bg-slate-50 border border-slate-200 p-2.5 sm:p-3 rounded-xl outline-none focus:border-[#059669] text-xs sm:text-sm cursor-pointer"
              onChange={(e) => setType(e.target.value)}
            >
              <option value="notes">Notes & PDF</option>
              <option value="quiz">Quiz Sheet</option>
          ...
          {/* Upload Box */}
          <div className="border-2 border-dashed border-slate-200 rounded-2xl p-5 sm:p-8 text-center hover:border-[#059669]/50 transition-colors">
            <input
              type="file"
              id="file-upload"
              className="hidden"
        ...
        <div className="p-4 sm:p-6 bg-slate-50 flex flex-col sm:flex-row gap-2 sm:gap-3">
          <button
            onClick={uploadMaterial}
            disabled={loading}
            className="w-full sm:flex-1 bg-[#059669] text-white py-2.5 sm:py-3 rounded-xl font-bold text-xs sm:text-sm hover:bg-[#047857] transition-all shadow-md active:scale-95 disabled:bg-emerald-200"
          >
            {loading ? "Uploading..." : "Confirm Upload"}
          </button>
          <button
            onClick={onClose}
            className="w-full sm:w-auto px-4 sm:px-6 py-2.5 sm:py-3 border border-slate-200 text-slate-600 rounded-xl font-bold text-xs sm:text-sm hover:bg-white transition-all"
          >
            Cancel
          </button>
        </div>
