...
const AdminAssignments = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axios.get("http://localhost:5000/api/admin/assignments-report", {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure? This will delete the assignment!")) {
      try {
        const token = localStorage.getItem("token");
        await axios.delete(`http://localhost:5000/api/admin/delete-assignment/${id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        fetchData(); 
      } catch (err) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(`http://localhost:5000/api/admin/update-assignment/${editAssign._id}`, 
