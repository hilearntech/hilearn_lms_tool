...
const AdminAssignments = () => {
  ...
  return (
    <div className="space-y-5 sm:space-y-8 relative font-sans">

      {/* HEADER */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 sm:p-6 md:p-8 rounded-2xl md:rounded-[32px] border border-slate-100 shadow-sm">

        <div>
          <h1 className="text-lg sm:text-xl md:text-2xl font-black text-slate-800 flex items-center gap-2 sm:gap-3">
            <ClipboardList className="text-[#059669]" size={20} /> Assignment Control
          </h1>
          <p className="text-slate-500 text-xs sm:text-sm font-medium mt-1">
            Manage all assignments with Emerald precision.
          </p>
        </div>
        ...
        <div className="relative w-full md:w-auto">
          ...
          <input
            type="text"
            placeholder="Search Mentor..."
            className="pl-10 sm:pl-12 pr-4 sm:pr-6 py-2.5 sm:py-3 bg-slate-50 rounded-2xl text-xs sm:text-sm font-bold w-full md:w-80 outline-none focus:ring-2 ring-emerald-100 text-slate-700"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6">
        {searchTerm.trim() === "" ? (
          <div className="text-center py-14 sm:py-20 bg-white rounded-2xl md:rounded-[32px] border-2 border-dashed border-slate-100 text-slate-400 font-bold text-xs sm:text-sm">
            <User className="mx-auto mb-3 sm:mb-4 opacity-20" size={36} />
            Type a Mentor's name to manage assignments
          </div>
        ) : filteredAssignments.map((assign) => (
          <div key={assign._id} className="bg-white rounded-2xl md:rounded-[2.5rem] border border-slate-100 overflow-hidden hover:shadow-md transition-all group">
            ...
            <div className="p-4 sm:p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 sm:gap-6">
              ...
              <div className="flex items-start sm:items-center gap-3 sm:gap-5">
                <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-emerald-50 flex items-center justify-center text-[#059669]">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="w-2 h-2 rounded-full bg-[#059669]"></span>
                    <p className="text-[10px] sm:text-xs font-bold text-[#059669] uppercase tracking-wider">
                      Mentor: {assign.mentor?.name}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-2 sm:gap-3 w-full md:w-auto justify-end">
                <button onClick={() => setSelectedAssign(assign)} className="p-2 sm:p-3 bg-slate-50 text-slate-600 rounded-xl text-xs sm:text-sm">
                  <Eye size={16} />
                </button>
                <button onClick={() => setEditAssign(assign)} className="p-2 sm:p-3 bg-slate-50 text-slate-600 rounded-xl">
                  <Edit3 size={16} />
                </button>
                <button onClick={() => handleDelete(assign._id)} className="p-2 sm:p-3 bg-slate-50 text-slate-400 rounded-xl">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
      ...
      {/* EDIT MODAL */}
      {editAssign && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-3 sm:p-4 bg-slate-900/40 backdrop-blur-sm">

          <div className="bg-white w-full max-w-[95%] sm:max-w-xl rounded-2xl md:rounded-[40px] shadow-2xl overflow-hidden p-4 sm:p-6 md:p-8 border border-emerald-50">
            ...
            <div className="flex justify-between items-center mb-6 sm:mb-8">
              ...
              <button onClick={() => setEditAssign(null)} className="p-1.5 sm:p-2 hover:bg-slate-100 rounded-full">
            ...
            <form onSubmit={handleUpdate} className="space-y-4 sm:space-y-6">
              <div>
                <label className="text-[9px] sm:text-[10px] font-black text-[#059669] uppercase tracking-widest ml-1">
                  Assignment Title
                </label>
                <input
                  type="text"
                  className="w-full mt-2 p-3 sm:p-4 bg-slate-50 rounded-2xl text-xs sm:text-sm font-bold text-slate-700 outline-none"
                  value={editAssign.title}
                  onChange={(e) => setEditAssign({ ...editAssign, title: e.target.value })}
                />
              </div>
              ...
              <button type="submit" className="w-full py-3 sm:py-4 bg-[#059669] text-white rounded-2xl font-black text-xs sm:text-sm flex items-center justify-center gap-2">
      ...
      {/* VIEW MODAL */}
      {selectedAssign && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-3 sm:p-4 bg-slate-900/40 backdrop-blur-sm">

          <div className="bg-white w-full max-w-[95%] sm:max-w-2xl rounded-2xl md:rounded-[40px] shadow-2xl p-4 sm:p-6 md:p-10 border border-emerald-50">
            ...
            <div className="flex justify-between items-start mb-6 sm:mb-8">
              <div>
                ...
                <p className="text-[#059669] text-[10px] sm:text-xs font-black uppercase mt-1 italic">
                  Review Mode
                </p>
              </div>

              <button onClick={() => setSelectedAssign(null)} className="p-1.5 sm:p-2 hover:bg-slate-100 rounded-full">
                <X size={20} className="text-slate-400" />
              </button>
            </div>

            <div className="bg-emerald-50/30 p-4 sm:p-6 md:p-8 rounded-2xl md:rounded-[2rem] text-xs sm:text-sm text-slate-600 mb-6 sm:mb-8 min-h-[120px] sm:min-h-[150px]">
              {selectedAssign.description || "No description provided for this assignment."}
            </div>

            <div className="flex gap-3 sm:gap-4">
              <button className="w-full py-3 sm:py-4 bg-slate-800 text-white rounded-2xl font-black text-xs sm:text-sm">
                Close Viewer
              </button>
            </div>
          </div>
