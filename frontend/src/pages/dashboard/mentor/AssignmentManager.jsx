...
//         axios.get("https://hilearnlmstool-production.up.railway.app/api/mentor/assignments", { headers: { Authorization: `Bearer ${token}` } }),
//         axios.get("https://hilearnlmstool-production.up.railway.app/api/mentor/my-lectures", { headers: { Authorization: `Bearer ${token}` } })
//       ]);
...
//       if (editingId) {
//         await axios.put(`https://hilearnlmstool-production.up.railway.app/api/mentor/update-assignment/${editingId}`, data, config);
//       } else {
//         await axios.post("https://hilearnlmstool-production.up.railway.app/api/mentor/create-assignment", data, config);
//       }
...
//     try {
//       const token = localStorage.getItem("token");
//       await axios.post("https://hilearnlmstool-production.up.railway.app/api/mentor/evaluate-assignment", {
...
//       try {
//         const token = localStorage.getItem("token");
//         await axios.delete(`https://hilearnlmstool-production.up.railway.app/api/mentor/delete-assignment/${id}`, {
//           headers: { Authorization: `Bearer ${token}` }
//         });
//         fetchData();
...
//                   <img src={`https://hilearnlmstool-production.up.railway.app${viewingAssignment.image}`} className="w-full h-auto max-h-72 object-contain bg-white" alt="reference" />
//                 </div>
//               )}
...
const AssignmentManager = () => {
  ...
  return (
    <div className="p-4 sm:p-5 md:p-6 max-w-6xl mx-auto space-y-6 sm:space-y-8 min-h-screen bg-white font-sans text-slate-900">

      {/* HEADER WITH STATS */}
      <div className="flex flex-col gap-4 border-b border-slate-100 pb-6 sm:pb-8 md:flex-row md:justify-between md:items-center md:gap-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-800 tracking-tight">Assignment Console</h1>
          <p className="text-sm sm:text-base font-bold text-emerald-600 tracking-widest mt-1">Manage Tasks & Evaluate Students</p>
        </div>

        <div className="flex items-center gap-3 sm:gap-4 w-full md:w-auto">
          {/* Stats — visible on sm and above */}
          <div className="flex gap-2 sm:gap-3 flex-1 md:flex-none md:mr-2">
            <div className="bg-emerald-50 border border-emerald-100 px-3 py-2 sm:px-4 sm:py-2.5 rounded-2xl flex items-center gap-1.5 sm:gap-2 shadow-sm flex-1 md:flex-none justify-center md:justify-start">
              <CheckSquare size={14} className="text-emerald-600 shrink-0" />
              <span className="text-xs sm:text-sm font-bold text-emerald-900 whitespace-nowrap">{totalTasks} Tasks</span>
            </div>
            <div className="bg-amber-50 border border-amber-100 px-3 py-2 sm:px-4 sm:py-2.5 rounded-2xl flex items-center gap-1.5 sm:gap-2 shadow-sm flex-1 md:flex-none justify-center md:justify-start">
              <Clock size={14} className="text-amber-600 shrink-0" />
              <span className="text-xs sm:text-sm font-bold text-amber-900 whitespace-nowrap">{pendingReviews} Pending</span>
            </div>
          </div>

          <button
            onClick={() => setShowForm(true)}
            className="bg-emerald-600 text-white px-4 py-3 sm:px-6 sm:py-3.5 md:px-8 md:py-4 rounded-2xl font-bold text-xs sm:text-sm hover:bg-slate-900 transition-all flex items-center justify-center gap-2 shadow-lg shadow-emerald-200 shrink-0"
          >
            <PlusCircle size={16} />
      ...
      <section className="space-y-4 sm:space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 sm:gap-8">
          {assignments.map((assign) => (
            <div key={assign._id} className="bg-white border-2 border-slate-50 rounded-[24px] sm:rounded-[32px] overflow-hidden shadow-sm hover:border-emerald-100 transition-all">

              {/* Assignment Card Header */}
              <div className="bg-slate-50/60 px-4 sm:px-6 md:px-8 py-4 sm:py-5 border-b border-slate-100 flex justify-between items-center gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="p-2 sm:p-3 bg-white rounded-2xl border border-slate-200 text-emerald-600 shadow-sm shrink-0">
                    <Calendar size={18} />
                  </div>
                  <div className="min-w-0">
                    <h3 className="text-base sm:text-lg font-bold text-slate-800 leading-none truncate">{assign.title}</h3>
                    <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mt-1.5 truncate">{assign.course?.title}</p>
                  </div>
                </div>
                <div className="flex gap-1.5 sm:gap-2 shrink-0">
                  <button onClick={() => setViewingAssignment(assign)} className="p-2 sm:p-3 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-emerald-600 hover:border-emerald-200 transition-all shadow-sm"><Eye size={16} /></button>
                  <button onClick={() => startEdit(assign)} className="p-2 sm:p-3 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-blue-600 hover:border-blue-200 transition-all shadow-sm"><Edit3 size={16} /></button>
                  <button onClick={() => handleDelete(assign._id)} className="p-2 sm:p-3 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-red-500 hover:border-red-200 transition-all shadow-sm"><Trash2 size={16} /></button>
                </div>
              </div>
              ...
              <div className="p-4 sm:p-5 md:p-6 space-y-3 sm:space-y-4">
                {assign.submissions.map((sub) => {
                  ...
                  return (
                    <div
                      key={sub.student?._id}
                      className="bg-slate-50/40 border border-slate-100 p-4 rounded-3xl flex flex-col gap-3 sm:gap-4 group hover:bg-white hover:shadow-md transition-all
                                 lg:flex-row lg:items-center lg:gap-6"
                    >
                      {/* Student Info */}
                      <div className="flex items-center gap-3 w-full lg:w-1/4">
                        <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-2xl flex items-center justify-center font-bold text-sm shadow-sm shrink-0 ${isGraded ? 'bg-emerald-600 text-white' : 'bg-white border border-slate-200 text-slate-400'}`}>
                        ...
                        {/* Open Work link — shown inline on sm/md next to student name */}
                        <a
                          href={sub.submissionLink}
                          target="_blank"
                          rel="noreferrer"
                          className="ml-auto lg:hidden flex items-center gap-1.5 px-3 py-2 bg-white border border-slate-200 rounded-2xl text-slate-600 font-bold text-[10px] uppercase hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all shadow-sm shrink-0"
                        >
                          <ExternalLink size={13} />
                          <span className="hidden sm:inline">Open Work</span>
                        </a>
                      </div>

                      {/* Open Work — hidden on sm/md, shown on lg */}
                      <a
                        href={sub.submissionLink}
                        target="_blank"
                        rel="noreferrer"
                        className="hidden lg:flex items-center gap-2 px-5 py-3 bg-white border border-slate-200 rounded-2xl text-slate-600 font-bold text-[10px] uppercase hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all shadow-sm"
                      >
                        <ExternalLink size={14} /> Open Work
                      </a>

                      {/* Feedback + Grade inputs */}
                      <div className="flex gap-2 sm:gap-3 w-full lg:flex-1">
                        <input
                          className="text-xs p-3 sm:p-4 border border-slate-200 rounded-[18px] flex-1 min-w-0 outline-none focus:border-emerald-500 bg-white shadow-inner"
                          placeholder="Mentor Feedback..."
                          value={currentEval.feedback}
                          onChange={(e) => setEvaluation({ ...evaluation, [`${assign._id}-${sub.student?._id}`]: { ...currentEval, feedback: e.target.value } })}
                        />
                        <input
                          className="text-xs p-3 sm:p-4 border border-slate-200 rounded-[18px] w-20 sm:w-24 text-center outline-none focus:border-emerald-500 font-bold uppercase bg-white shadow-inner shrink-0"
                          placeholder="Grade"
                          value={currentEval.grade}
                          onChange={(e) => setEvaluation({ ...evaluation, [`${assign._id}-${sub.student?._id}`]: { ...currentEval, grade: e.target.value } })}
                        />
                      </div>

                      {/* Submit Grade button */}
                      <button
                        onClick={() => handleEvaluate(assign._id, sub.student?._id)}
                        className={`w-full lg:w-auto px-5 sm:px-8 py-3 sm:py-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest transition-all shadow-sm ${isGraded ? 'bg-slate-900 text-white hover:bg-black' : 'bg-emerald-600 text-white hover:bg-emerald-700'}`}
                      >
                        {isGraded ? "Update" : "Submit Grade"}
                      </button>
                    </div>
                ...
                {assign.submissions.length === 0 && (
                  <div className="py-10 sm:py-12 flex flex-col items-center text-slate-300">
      ...
      {showForm && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white w-full sm:max-w-2xl rounded-t-[32px] sm:rounded-[40px] shadow-2xl overflow-hidden flex flex-col max-h-[95vh] sm:max-h-[92vh]">

            <div className="p-5 sm:p-8 border-b flex justify-between items-center bg-white sticky top-0 z-10">
              <h2 className="font-extrabold text-slate-900 text-lg sm:text-xl tracking-tight">{editingId ? "Edit Assignment" : "Create New Task"}</h2>
              <button onClick={resetForm} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors"><X size={22} /></button>
            </div>

            <div className="overflow-y-auto p-5 sm:p-8 sm:pt-4 custom-scrollbar">
              <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">

                {/* Title + Deadline — stacked on sm, side by side on md+ */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-2">Task Title</label>
                    <input
                      type="text"
                      className="w-full p-3.5 sm:p-4 bg-slate-50 border border-slate-100 rounded-[20px] font-semibold text-sm outline-none focus:bg-white focus:border-emerald-500 transition-all shadow-sm"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-2">Associated Course</label>
                  <select
                    className="w-full p-3.5 sm:p-4 bg-slate-50 border border-slate-100 rounded-[20px] font-semibold text-sm outline-none focus:bg-white focus:border-emerald-500 transition-all shadow-sm appearance-none"
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    rows="4"
                    className="w-full p-3.5 sm:p-4 bg-slate-50 border border-slate-100 rounded-[24px] font-medium text-sm outline-none focus:bg-white focus:border-emerald-500 transition-all shadow-sm resize-none"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-2">Reference Image (Optional)</label>
                  <label className="flex items-center justify-between p-4 sm:p-5 bg-emerald-50/30 border-2 border-dashed border-emerald-100 rounded-[24px] cursor-pointer hover:border-emerald-400 hover:bg-emerald-50/50 transition-all">
                    <span className="text-xs font-semibold text-emerald-700 truncate pr-3">{assignmentImage ? assignmentImage.name : "Click to upload helper image..."}</span>
                ...
                <div className="pt-2 pb-4">
                  <button
                    type="submit"
                    className="w-full py-4 sm:py-5 bg-emerald-600 text-white rounded-[24px] font-bold uppercase tracking-widest hover:bg-slate-900 shadow-xl shadow-emerald-100 transition-all active:scale-[0.98]"
                  >
                    {editingId ? "Save Changes" : "Publish Assignment"}
                  </button>
                </div>
      ...
      {viewingAssignment && (
        <div className="fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white w-full sm:max-w-xl rounded-t-[32px] sm:rounded-[40px] p-6 sm:p-10 shadow-2xl border border-slate-100 max-h-[90vh] overflow-y-auto">

            <div className="flex justify-between items-start mb-5 sm:mb-6 gap-3">
              <div className="min-w-0">
                <h2 className="text-xl sm:text-2xl font-bold text-slate-900 leading-snug">{viewingAssignment.title}</h2>
                <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mt-1.5">{viewingAssignment.course?.title}</p>
              </div>
              <button onClick={() => setViewingAssignment(null)} className="p-2 bg-slate-50 rounded-full text-slate-400 hover:bg-slate-100 shrink-0"><X size={20} /></button>
            </div>

            <div className="space-y-5 sm:space-y-6">
              <div className="bg-slate-50 p-4 sm:p-6 rounded-[20px] sm:rounded-[24px] text-sm text-slate-600 font-medium leading-relaxed border border-slate-100">
                {viewingAssignment.description}
              </div>

              {viewingAssignment.image && (
                <div className="rounded-[20px] sm:rounded-[24px] overflow-hidden border-4 border-white shadow-lg">
                  <img
                    src={`https://hilearnlmstool-production.up.railway.app${viewingAssignment.image}`}
                    className="w-full h-auto max-h-64 sm:max-h-72 object-contain bg-white"
                    alt="reference"
                  />
                </div>
              )}
              ...
              <button
                ...
                className="w-full py-3.5 sm:py-4 bg-slate-900 text-white rounded-[20px] font-bold uppercase tracking-widest hover:bg-emerald-600 transition-all"
              >
                Close
              </button>
            </div>
