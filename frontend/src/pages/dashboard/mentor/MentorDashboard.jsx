...
const MentorDashboard = () => {
  ...
  <p className="text-slate-500 mb-6 sm:mb-8 text-sm sm:text-base font-medium">
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-10">
    ...
    {/* Students */}
    <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-3 sm:gap-4">
      <div className="bg-emerald-100 text-emerald-600 p-2 sm:p-3 rounded-xl">
        <Users size={20} />
      </div>
      <div>
        <p className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase">
    ...
    {/* Next Session */}
    <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-3 sm:gap-4 sm:col-span-2 lg:col-span-1">
      <div className="bg-amber-100 text-amber-600 p-2 sm:p-3 rounded-xl">
        <Clock size={20} />
      </div>
      <div>
        <p className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase">
        ...
        <h3 className="text-xs sm:text-sm font-bold text-slate-800">
          {upcomingLive[0] ? (
            <span className="flex flex-col">
              <span>
                {new Date(upcomingLive[0].date).toLocaleDateString("en-IN", {
                  day: "numeric",
                  month: "short",
                })}
              </span>
              <span className="text-[#059669]">
                {upcomingLive[0].startTime}
              </span>
            </span>
          ) : (
            "No Sessions"
  ...
  <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-100 shadow-sm p-4 sm:p-6">
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5 sm:mb-6">
      <h3 className="text-base sm:text-lg font-bold text-slate-800 flex items-center gap-2">
        <Calendar className="text-[#059669]" size={18} /> Upcoming Live Classes
      </h3>
    </div>
    ...
    {upcomingLive.length > 0 ? (
      <div className="space-y-3 sm:space-y-4">
        {upcomingLive.map((live) => (
          <div
            key={live._id}
            className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 p-3 sm:p-4 bg-slate-50 rounded-2xl border border-slate-100"
          >
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] sm:text-xs text-slate-500 font-medium">
                  {live.course?.title} • {live.date || "TBD"}
                </p>
              </div>
            </div>
            ...
            <button
              onClick={() => window.open(live.meetingLink, "_blank")}
              className="w-full sm:w-auto px-4 py-2 bg-[#059669] text-white text-xs font-bold rounded-lg hover:bg-[#047857] transition-all flex items-center justify-center gap-2"
            >
              Join <ExternalLink size={14} />
            </button>
          </div>
      ...
      <div className="text-center py-8 sm:py-10">
        <p className="text-slate-400 text-xs sm:text-sm italic">
          No live classes scheduled for today or tomorrow.
        </p>
      </div>
    )}
  </div>
