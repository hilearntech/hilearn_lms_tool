...
const ManageAttendance = () => {
  ...
  const fetchMentorLectures = async () => {
    ...
    try {
      ...
      const res = await axios.get("https://hilearnlmstool-production.up.railway.app/api/mentor/my-assigned-lectures", {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      ...
      try {
        const token = localStorage.getItem("token");
        const res = await axios.get(`https://hilearnlmstool-production.up.railway.app/api/attendance/filtered-students/${selectedLecture}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (res.data.success) {
  ...
  const saveAttendance = async () => {
    ...
    try {
      ...
      await axios.post("https://hilearnlmstool-production.up.railway.app/api/attendance/bulk",
  ...
  return (
    ...
    <div className="p-3 sm:p-4 md:p-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-start sm:items-center justify-between mb-6 sm:mb-8 gap-3">
        <div>
          ...
          <p className="text-slate-500 text-sm sm:text-base font-medium">
            Mark presence for your assigned sessions
          </p>
        </div>

        <button
          onClick={fetchMentorLectures}
          className="p-2 sm:p-3 hover:bg-slate-100 rounded-full transition-colors"
        >
      ...
      {/* SELECT */}
      <div className="bg-emerald-600 p-4 sm:p-6 md:p-8 rounded-2xl sm:rounded-[32px] mb-6 sm:mb-8 shadow-lg sm:shadow-xl shadow-emerald-100">

        <label className="text-emerald-100 text-[9px] sm:text-[10px] font-black uppercase tracking-widest mb-2 sm:mb-3 block">
          Choose Lecture Session
        </label>

        <select
          className="w-full bg-white/10 border border-white/20 p-3 sm:p-4 rounded-xl sm:rounded-2xl text-white text-sm sm:text-base font-bold outline-none focus:bg-white/20 transition-all cursor-pointer"
          value={selectedLecture}
          onChange={(e) => setSelectedLecture(e.target.value)}
        >
          <option className="text-slate-800" value="">
            Select a session from your list...
          </option>
          {lectures.map((l) => (
            <option className="text-slate-800" key={l._id} value={l._id}>
              {l.title}
            </option>
          ))}
        </select>
      ...
      <div className="bg-white rounded-2xl sm:rounded-[40px] border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[600px]">
            ...
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                <th className="px-4 sm:px-8 py-4 sm:py-6 text-left text-[9px] sm:text-[11px] font-black text-slate-400 uppercase">
                  Student Profile
                </th>
                <th className="px-4 sm:px-8 py-4 sm:py-6 text-center text-[9px] sm:text-[11px] font-black text-slate-400 uppercase">
                  Current Status
                </th>
                <th className="px-4 sm:px-8 py-4 sm:py-6 text-right text-[9px] sm:text-[11px] font-black text-slate-400 uppercase">
                  Action
                </th>
              </tr>
            </thead>
              ...
              {filtering ? (
                ...
                students.map((student) => (
                  <tr key={student._id} className="group hover:bg-slate-50/50 transition-colors">
                    ...
                    {/* NAME */}
                    <td className="px-4 sm:px-8 py-4 sm:py-6 font-bold text-slate-700 text-sm">
                      {student.name}
                    </td>
                    ...
                    <td className="px-4 sm:px-8 py-4 sm:py-6 text-center">
                      <span
                        className={`px-3 sm:px-4 py-1 rounded-full text-[9px] sm:text-[10px] font-black uppercase ${attendance[student._id] === "present"
                    ...
                    <td className="px-4 sm:px-8 py-4 sm:py-6">
                      <div className="flex justify-end gap-2">
                        ...
                        <button
                          ...
                          className={`p-2 rounded-lg sm:rounded-xl border transition-all ${attendance[student._id] === "present"
                              ? "bg-emerald-500 border-emerald-500 text-white shadow-lg"
                              : "bg-white border-slate-200 text-slate-300 hover:border-emerald-500"
                            }`}
                        >
                          <CheckCircle size={18} />
                        </button>
                        ...
                        <button
                          ...
                          className={`p-2 rounded-lg sm:rounded-xl border transition-all ${attendance[student._id] === "absent"
                              ? "bg-red-500 border-red-500 text-white shadow-lg"
                              : "bg-white border-slate-200 text-slate-300 hover:border-red-500"
                            }`}
                        >
                          <XCircle size={18} />
                        </button>
                ...
                <tr>
                  <td colSpan="3" className="py-16 sm:py-20 text-center text-slate-400 font-bold text-sm">
      ...
      <div className="mt-6 sm:mt-8 flex justify-end">
        <button
          onClick={saveAttendance}
          disabled={!selectedLecture || loading || students.length === 0}
          className="w-full sm:w-auto justify-center bg-slate-900 text-white px-6 sm:px-12 py-3 sm:py-4 rounded-xl sm:rounded-[20px] font-black text-xs sm:text-sm flex items-center gap-2 sm:gap-3 hover:bg-[#059669] transition-all disabled:opacity-50 active:scale-95 shadow-xl sm:shadow-2xl shadow-slate-200"
        >
          {loading ? (
