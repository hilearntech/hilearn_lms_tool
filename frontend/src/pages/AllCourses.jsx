...
//     try {
//       const response = await fetch("https://hilearnlmstool-production.up.railway.app/api/courses");
...
const CoursesPage = () => {
  ...
  return (
    // ✅ overflow-x-hidden add kiya — horizontal scroll band
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-gray-200">
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-white border-t border-gray-200">
            <div className="px-4 py-4 space-y-1">
              {menuItems.map((item) => (
                <a key={item.name} href={item.path} onClick={() => setMobileMenuOpen(false)}
                  className="block px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-lg cursor-pointer"
                >
                  {item.name}
                </a>
              ...
              <div className="pt-4 space-y-2">
                <button onClick={() => navigate("/login")} className="w-full py-2.5 text-sm font-semibold text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-50">Sign In</button>
                <button onClick={() => navigate("/register")} className="w-full py-2.5 bg-emerald-600 text-white text-sm font-semibold rounded-lg hover:bg-emerald-700">Get Started</button>
              </div>
            </div>
          </div>
        )}
      ...
      {/* PAGE HEADER */}
      {/* ✅ SM/MD: py reduced, heading size adjusted */}
      <section className="bg-emerald-600 text-white py-10 sm:py-14 lg:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
      ...
      {/* FILTERS & SEARCH */}
      <section className="bg-gray-50 border-b border-gray-200 py-5 sm:py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* ✅ SM: column stack, MD/LG: row */}
          <div className="flex flex-col sm:flex-col lg:flex-row gap-3 sm:gap-4">
            ...
            {/* ✅ SM/MD: filters side by side in a row */}
            <div className="flex gap-3 sm:gap-4 w-full lg:w-auto">
              ...
              <div className="relative flex-1 lg:flex-none">
                <select
                  value={selectedCategory}
                  onChange={(e) => setSelectedCategory(e.target.value)}
                  className="appearance-none w-full lg:w-auto px-4 py-2.5 pr-10 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-emerald-600 bg-white cursor-pointer"
                >
                  {categories.map(cat => (
                    <option key={cat} value={cat}>{cat}</option>
                  ))}
      ...
      {/* COURSES GRID */}
      {/* ✅ SM: 1-col, MD: 2-col, LG: 3-col same */}
      ...
      <section className="py-12 sm:py-14 lg:py-16 bg-gray-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <button onClick={() => navigate("/contact")} className="px-8 py-3.5 bg-emerald-600 text-white font-semibold rounded-lg hover:bg-emerald-700 transition-colors">
            Book Free Consultation
          </button>
        </div>
      </section>
