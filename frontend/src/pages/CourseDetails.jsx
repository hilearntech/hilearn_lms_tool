  ...
  useEffect(() => {
    const fetchCourseDetail = async () => {
      try {
        ...
        const response = await fetch(`https://hilearnlmstool-production.up.railway.app/api/courses/${id}`);
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center text-gray-600 hover:text-emerald-600 text-sm sm:text-base"
          >
            <ArrowLeft className="w-5 h-5 mr-2" /> Back
          </button>
        </div>
      ...
      {/* Main Layout */}
      <div className="max-w-7xl mx-auto px-4 py-6 sm:py-8 md:py-12 grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 md:gap-12">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            {(typeof course.features[0] === 'string'
              ...
              <div
                key={i}
                className="flex items-center gap-3 bg-white p-3 sm:p-4 rounded-lg border border-gray-200"
              >
                <CheckCircle2 className="text-emerald-500 w-5 h-5 flex-shrink-0" />
                <span className="text-sm sm:text-base">{f.trim()}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Right Sidebar */}
        <div className="lg:col-span-1">
          <div className="bg-white p-5 sm:p-6 md:p-8 rounded-2xl border border-gray-200 shadow-xl lg:sticky lg:top-24">
            ...
            <div className="flex justify-between items-center mb-4 sm:mb-6">
              ...
              <div className="flex items-center bg-yellow-100 px-2 py-1 rounded text-yellow-700 font-bold text-xs sm:text-sm">
                <Star className="w-4 h-4 fill-yellow-500 mr-1" /> {course.rating}
              </div>
            </div>
            ...
            <div className="space-y-3 sm:space-y-4 mb-6 sm:mb-8">
              <div className="flex items-center text-gray-600 text-sm sm:text-base">
                <Clock className="w-5 h-5 mr-3" /> {course.duration}
              </div>
              <div className="flex items-center text-gray-600 text-sm sm:text-base">
                <Users className="w-5 h-5 mr-3" /> {course.studentCount} Students
              </div>
            </div>

            {/* Buttons */}
            <button className="w-full py-3 sm:py-4 bg-emerald-600 text-white rounded-xl font-bold mb-3 sm:mb-4 hover:bg-emerald-700 transition-all text-sm sm:text-base">
              Enroll Now
            </button>

            <button
              onClick={() => navigate(`/demo-request/${course._id}`)}
              className="w-full py-3 sm:py-4 border-2 border-emerald-600 text-emerald-600 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-emerald-50 transition-all text-sm sm:text-base"
            >
              <PlayCircle className="w-5 h-5" /> Watch Demo Video
            </button>
          </div>
