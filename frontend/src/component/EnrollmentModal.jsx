...
const EnrollmentModal = ({ course, onClose, onEnrollSuccess }) => {
  ...
  const handleSignupSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const { data } = await axios.post("https://hilearnlmstool-production.up.railway.app/api/auth/send-otp", {
  ...
  const handleFinalPayment = async () => {
    setIsLoading(true);
    try {
      const { data } = await axios.post("https://hilearnlmstool-production.up.railway.app/api/auth/quick-enroll", {
        name: formData.name,
        email: formData.email,
